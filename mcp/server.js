...
function pathTokenReady() {
  // Render's generateValue uses standard Base64, which can contain +, / and
  // trailing = padding. URL-safe tokens remain valid as well.
  return /^[A-Za-z0-9+\/_-]{24,}={0,2}$/.test(MCP_PATH_TOKEN);
}
...
function requireMcpPathToken(req, res, next) {
  if (!pathTokenReady()) {
    return res.status(503).json({
      ...
      message: "Set LINJIAN_MCP_PATH_TOKEN to at least 24 Base64 or URL-safe characters."
    });
  }
  // RegExp routes capture the whole remaining path, including any '/' inside
  // a standard Base64 token generated by Render.
  if (!tokenMatches(req.params[0])) return res.status(404).end();
  next();
}

const MCP_PRIVATE_ROUTE = /^\/mcp\/(.+)$/;
const MCP_WALLET_PRIVATE_ROUTE = /^\/mcp-wallet\/(.+)$/;
const LEGACY_SSE_PRIVATE_ROUTE = /^\/sse\/(.+)$/;
const LEGACY_MESSAGES_PRIVATE_ROUTE = /^\/messages\/(.+)$/;
...
app.post(MCP_PRIVATE_ROUTE, requireMcpPathToken, async (req, res) => {
...
app.get(MCP_PRIVATE_ROUTE, requireMcpPathToken, (_req, res) => res.status(405).json({ ok: false, error: "Use POST for Streamable HTTP MCP." }));
app.post(MCP_WALLET_PRIVATE_ROUTE, requireMcpPathToken, (req, res, next) => {
...
app.get(LEGACY_SSE_PRIVATE_ROUTE, requireMcpPathToken, (req, res, next) => {
...
app.post(LEGACY_MESSAGES_PRIVATE_ROUTE, requireMcpPathToken, (req, res, next) => {
  if (!ENABLE_LEGACY_SSE) return res.status(404).end();
  next();
